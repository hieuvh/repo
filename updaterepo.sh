#!/usr/bin/env bash
if [[ "$OSTYPE" == "linux"* ]]; then # Linux usage of Cypous
    cd "$(dirname "$0")" || exit
    ...
    echo "Repo updated, thanks for using Cypous!"
    elif [[ "$(uname)" == Darwin ]] && [[ "$(uname -p)" == i386 ]]; then # macOS usage of Cypous
    cd "$(dirname "$0")" || exit
    ...
    echo "Running an unsupported os...? Contact me via Twitter @helios017" 
fi
